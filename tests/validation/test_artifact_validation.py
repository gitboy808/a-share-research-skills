...
class WorkspaceArtifactValidationTests(unittest.TestCase):
    ...
    def write_historical_run_with_workset(self) -> tuple[Path, Path]:
        run_path = self.workspace / "运行记录/2026-08/RUN-20260801-001.md"
...
""",
        ...
        workset_path = (
            self.workspace
            / "运行记录/2026-08/RUN-20260801-001-historical-unknown-工作集清单.json"
        )
        legacy_payload = {
            "stable_references": [],
            "relations": [],
        ...
        workset = {
            ...
            "id": "RUN-20260801-001-WORKSET-HISTORICAL-UNKNOWN",
            "attempt": 1,
            "status": "partial",
            ...
            "created_at": "当时未记录",
            "information_cutoff": "当时未记录",
            "task_contract": {"contract_id": "unknown", "version": "unknown"},
            ...
            "gaps": [{"reason": "historical_workset_not_recorded", "blocking": True}],
            "quality": {
                "assembled_units": 0,
            ...
            "legacy_snapshot": {
                **legacy_payload,
                ...
                "canonical_sha256": legacy_payload_hash,
            },
        }
        workset_path.write_text(
            json.dumps(workset, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
        )
        return run_path, workset_path
    ...
    def test_historical_record_allows_only_the_controlled_unknown_timestamp(self) -> None:
        path = self.workspace / "报告/迁移审计/迁移-判断.md"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            """---
schema_version: "a-share-workspace-v3"
artifact_type: "historical_record"
id: "HIST-JUDGMENT-TEST"
status: "historical"
created_at: "当时未记录"
information_cutoff: "当时未记录"
record_kind: "judgment_log_migration"
authority: "migration_audit"
migration_note: "仅供迁移审计"
...
""",
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace('created_at: "当时未记录"', 'created_at: "去年"'),
            encoding="utf-8",
        )
        self.assert_workspace_error(
            "报告/迁移审计/迁移-判断.md: historical created_at must be ISO-8601 with timezone or 当时未记录"
        )

    def test_historical_run_and_workset_allow_unknown_time_only_as_a_fail_closed_pair(self) -> None:
        self.write_historical_run_with_workset()

        self.assert_valid_workspace(warnings=False)

    def test_historical_workset_unknown_time_rejects_a_completed_verification_disguise(self) -> None:
        _, workset_path = self.write_historical_run_with_workset()
        manifest = json.loads(workset_path.read_text(encoding="utf-8"))
        ...
        self.assert_workspace_error(
            "运行记录/2026-08/RUN-20260801-001-historical-unknown-工作集清单.json: historical migration workset must remain partial/not_run/blocking with zero coverage"
        )

    def test_historical_workset_rejects_legacy_snapshot_hash_tampering(self) -> None:
        _, workset_path = self.write_historical_run_with_workset()
        manifest = json.loads(workset_path.read_text(encoding="utf-8"))
        manifest["legacy_snapshot"]["gaps"] = [
            {"reason": "tampered historical gap"}
        ]
        workset_path.write_text(
        ...
        self.assert_workspace_error(
            "运行记录/2026-08/RUN-20260801-001-historical-unknown-工作集清单.json: legacy_snapshot canonical_sha256 mismatch"
        )

    def test_live_workset_rejects_a_legacy_snapshot(self) -> None:
        manifest_path = self.write_partial_runtime_workset()
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest["legacy_snapshot"] = {
            "stable_references": [],
            "relations": [],
            "gaps": [],
            "coverage": {},
            ...
            "canonical_sha256": hashlib.sha256(
                b'{"coverage":{},"gaps":[],"relations":[],"stable_references":[]}'
            ).hexdigest(),
        }
        ...
        self.assert_workspace_error(
            "运行记录/2026-08/RUN-20260809-001-investigate-research-工作集清单.json: live workset must not contain legacy_snapshot"
        )
    ...
    def test_historical_record_cannot_be_used_as_a_new_unowned_artifact(self) -> None:
        path = self.workspace / "报告/2026-08/legacy-shaped-new-report.md"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            """---
schema_version: "a-share-workspace-v3"
...
status: "historical"
created_at: "2026-08-09T10:30:00+08:00"
information_cutoff: "2026-08-09T10:30:00+08:00"
record_kind: "报告"
---
...
""",
        ...
        payload = self.assert_workspace_error(
            "报告/2026-08/legacy-shaped-new-report.md: missing frontmatter field authority"
        )
        self.assertIn(
            "报告/2026-08/legacy-shaped-new-report.md: missing frontmatter field migration_note",
            payload["errors"],
        )
