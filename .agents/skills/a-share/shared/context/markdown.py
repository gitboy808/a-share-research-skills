...
def _authority(path: Path) -> str:
    ...
    if "观察日志" in parts or path.name == "观察池.md":
...
def _unit_type(path: Path, section: dict[str, Any], metadata: dict[str, Any]) -> str | None:
    ...
    if authority == "research_rules" and re.match(r"R\d{2}\b", title):
        return "rule"
    if authority == "object_dossier" and section.get("level", 0) >= 2:
        return "object_field"
...
def _stable_id(path: Path, section: dict[str, Any], metadata: dict[str, Any], unit_type: str) -> str:
    title = str(section.get("title", ""))
...
def extract_units(root: Path, path: Path) -> list[dict[str, Any]]:
...
def _make_unit(
    ...
    unit_id = _stable_id(path, section, metadata, unit_type)
